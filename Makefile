...
.PHONY: run
run:
	docker exec nextcloud-app-1 sudo -u www-data php occ app_api:app:unregister $(APP_ID) --silent --force || true
	docker exec nextcloud-app-1 sudo -u www-data php occ app_api:app:register $(APP_ID) --force-scopes \
		--info-xml https://raw.githubusercontent.com/cloud-py-api/nc_py_api/main/examples/as_app/$(APP_ID)/appinfo/info.xml
...
register:
	docker exec nextcloud-app-1 sudo -u www-data php occ app_api:app:unregister $(APP_ID) --silent --force || true
	docker exec nextcloud-app-1 sudo -u www-data php occ app_api:app:register $(APP_ID) manual_install --json-info $(JSON_INFO) --force-scopes --wait-finish
